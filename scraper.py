...
def scrape_site(start_url, output_dir, follow_pagination=True, max_pages=MAX_PAGES_TO_CRAWL):
    """
    Scrapes a website starting from start_url for images.

    Args:
        start_url (str): The URL to start scraping from.
        output_dir (str): The directory to save images to.
    ...
    output_path = os.path.join(output_dir, base_domain)
        ...
        try:
            ...
            for img_tag in soup.find_all(['img', 'source']):
                ...
                if img_tag.name == 'img':
                    ...
                    if not src: src = img_tag.get('data-lazy-src')
                ...
                try:
                    ...
                    # Get filename
                    img_filename = os.path.basename(urlparse(cleaned_img_url).path)
                    if not img_filename or '.' not in img_filename: # Handle URLs ending in / or no extension
                        ...
                        img_filename = f"image_{len(found_image_srcs)}{ext}"
            ...
            for link in soup.find_all('a', href=True):
                ...
                # Check if it's a pagination link (heuristic)
                link_text = link.get_text(strip=True).lower()
                link_rel = link.get('rel', [])
                link_class = link.get('class', [])
                is_pagination = (
                    'next' in link_text or '>' in link_text or '>>' in link_text or
                    'next' in link_rel or
                    any('page' in c.lower() or 'pagin' in c.lower() or 'next' in c.lower() for c in link_class)
                )

                # Add to crawl queue if it's new and allowed
                if abs_link not in visited_urls:
                    # Prioritize pagination links if enabled
                    if follow_pagination and is_pagination:
                         if len(urls_to_visit) + len(visited_urls) < max_pages: # Check before adding
                            logging.debug(f"Found potential pagination link: {abs_link}")
                            visited_urls.add(abs_link)
                            urls_to_visit.appendleft(abs_link) # Add to front of queue
    ...
    logging.info(f"Scraping finished. Crawled {pages_crawled} pages. Found {len(downloaded_image_hashes)} unique images.")
    return output_path, len(downloaded_image_hashes)
